...
Docker was first released in 2013 by Solomon Hykes and has since become a standard tool for containerization. It helps solve the common problem of “It works on my computer, but not on yours.” With Docker, apps run the same way everywhere whether it's on your laptop, a test server, or in the cloud.Key features include:
...
- Agar host machine Linux hai → container Linux kernel use karega
...
- Containers don’t need a full OS for each app.
...
• **Client**: Sends commands (CLI, GUI, API)
- The other part is the server (does the work and gives the result).
• **Server (Daemon)**: Executes commands and manages Docker objects
...
![How Docker Works](https://github.com/Yash2141/Docker-container/blob/main/docker-content/Docker%20201c5c1d487680848b19c244e5b8dca5/how-does-docker-work.png?raw=true)
...
if you don’t attach to it (`-it`) or give it a command, the container may start and exit immediately, because there's nothing to keep it running.
...
/etc/os-release It’s a file that contains information about the operating system inside the container
 ...
 **Execute Commands Inside a Running Container:** Opens an interactive terminal inside the container that’s already running.
...
**Docker Export:** Exports the entire container’s file system as a `.tar` archive.
...
A Docker Volume is a persistent storage mechanism used by Docker containers to store data outside the container’s writable layer. This means:
